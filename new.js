...
function isDocument() {
  return is.defined(globalThis.document)
}
...
function hasDotRoot(str) {
  return str === ".." || str === "." || str === ""
}

function _norm(url = "") {
  validateString(url)
  const segments = List.fromArray(url.split(regExpSep))
  const root = []
  const normSegments = []
  let seg = null

  while (segments.length > 0) {
    seg = segments.shift().value.trim()
    const sign = get_sign(seg)

    if (normSegments.length === 0 && !isRoot(root[0])) {
      const r = root[0]
      switch (sign) {
        case (-1): {
          if (r === "") { break }
        ...
        case (0): {
          if (hasDotRoot(r)) { break }
          root.push(seg)
          break
        }
        case (1): {
          if (isRoot(seg) && root.length === 0) {
            root.push(seg)
          } else {
  ...
  return root.concat(normSegments).join(sep)
}
